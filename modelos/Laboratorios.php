...
/////////////////ACEPTAR ORDENES /////////////
public function aprobar_ordenes(){
	$conectar = parent::conexion();
	parent::set_names();
	
	$detalle_envio = array();
	$detalle_envio = json_decode($_POST["arrayDetAprobar"]);
	...
	foreach ($detalle_envio as $k => $v) {
        ...
        $sql3 = "update ordenes_lab set estado='3' where id_orden_lab=? and cod_orden=? and paciente=?;";
        $sql3=$conectar->prepare($sql3);
		$sql3->bindValue(1,$id_orden_lab);
		$sql3->bindValue(2,$codigo);
		$sql3->bindValue(3,$paciente);
	...
	}////////////FIN FOREACH RECORRE APROBACION DE ORDENS
...
//||°°°°°°°°°°°ELIMINAR ORDEN DE LABORATORIO°°°°°||||
   public function eliminar_orden_lab($id_orden_lab){
    $conectar= parent::conexion();
    $sql="select*from ordends_lab where id_orden_lab=?;";
    $sql=$conectar->prepare($sql);
    $sql->bindValue(1, $id_orden_lab);
    $sql->execute();
    return $resultado= $sql->fetchAll(PDO::FETCH_ASSOC);
  }
